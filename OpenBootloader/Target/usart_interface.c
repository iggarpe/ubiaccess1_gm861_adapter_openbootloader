...
static void OPENBL_USART_Init(void)
{
  ...
  LL_USART_EnableDEMode(USARTx);

  // DE polarity: 0 = DE active high, 1 = DE active low
  LL_USART_SetDESignalPolarity(USARTx, LL_USART_DE_POLARITY_HIGH);

  // Set DE assertion and de-assertion times (in bit periods)
  // These values depend on your transceiver timing requirements
  LL_USART_SetDEAssertionTime(USARTx, 0);  	// DE asserted 0 bit times before TX
  LL_USART_SetDEDeassertionTime(USARTx, 0);	// DE deasserted 0 bit times after TX
...
void OPENBL_USART_Configuration(void)
{
  ...
  GPIO_InitStruct.Pull      = GPIO_PULLUP;
  GPIO_InitStruct.Speed     = GPIO_SPEED_FREQ_HIGH;
  GPIO_InitStruct.Alternate = USARTx_DE_ALTERNATE;
